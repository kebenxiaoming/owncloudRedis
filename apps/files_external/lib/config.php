...
class OC_Mount_Config {
	 ...
	 * 目前修改为只是搜索user分组下的信息。并且从数据库根据$user的名字进行查询，没有遍历
...
//		return array();
		...
		if(isset($user)) {
			...
			if(!empty($result)) {
				...
				$mountpoints = array('user' => array($result['name'] => array(
					$result['storage_name'] => array(
						'class' => $result['class'],
						'options' => $options,
		...
		if (isset($data[$mountType])) {
			if (isset($data[$mountType][$applicable])) {
				...
				//$data[$mountType] = array_merge($data[$mountType], $mountPoint);
                                $data[$mountType] = $data[$mountType]+$mountPoint;
			}
		} else {
